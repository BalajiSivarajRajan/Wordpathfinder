...
def wordpath(words, start, end):
    ...
    try:
                ...
                open_set = set([start])
                open_heap = [Node(hscore(start), 0, start, None)]
                while open_heap:
                        ...
                        for neighbourhood in neighbours[node.word]:
                                for w in neighbourhood:
                                        if w not in closed_set and w not in open_set:
                                                next_node = Node(hscore(w) + g, g, w, node)
...
def main():
    try:
                ...
                res = wordpath (readfile (args['dict']), args['start'], args['end'] )
                print res
    except:
